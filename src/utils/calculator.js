...
export function evaluate(expr) {
  ...
  let __log_pairs = []
  let __prev_log = ''
  // 优先匹配 logₐ( 双参数(visual hint 给用户看); 后匹配 log( 双参数
  // 单 log( 在下一步会被转成 log10(
  const logPairPattern = /\blogₐ\(\s*([^,()]+)\s*,\s*([^()]+)\)|\blog\(\s*([^,()]+)\s*,\s*([^()]+)\)/g
  while (normalized !== __prev_log) {
    __prev_log = normalized
    normalized = normalized.replace(logPairPattern, function (match, ab1, val1, ab2, val2) {
      const base = (ab1 || ab2).trim()
      const val = (val1 || val2).trim()
      // 如果 val 包含 log( 或 logₐ(, 跳过让内层先处理
      if (/\blog(?:ₐ)?\(/.test(val)) return match
      const idx = __log_pairs.length
      __log_pairs.push({ base, val })
  ...
  normalized = normalized
    ...
    // 注意:不能匹配 logₐ( (那是被占位符替换过的双参数 log)
    // 用 lookahead 否定: log( 后面不能是 a
    .replace(/\blog(?!ₐ)\(/g, 'log10(')
    // logₐ( → log( (变成标准 mathjs 形式,前面的两步处理会走双参数路径)
    .replace(/\blogₐ\(/g, 'log(')
    // ln(x) → log(x) (自然对数)
    .replace(/\bln\b/g, 'log')

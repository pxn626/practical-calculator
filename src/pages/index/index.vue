    ...
    const onKeyTap = (value) => {
      switch (value) {
        ...
        case "log":
          ...
          break
        case "logab":
          // 双参数 logₐ(a, b) = log_a(b) (以 a 为底,b 为真数)
          // 用 logₐ(...) 视觉提示,calculator.js 会转换
          expression.value += "logₐ("
          break
        case "ln":
          expression.value += "ln("
          break
        ...
        case ".":
          // 防重复小数点:当前数字已有小数点则忽略
          const lastNum = expression.value.split(/[+\-×÷^()%!πe]/).pop()
          if (!lastNum.includes(".")) expression.value += "."
          break
        default:
...
.landscape {
  ...
  // 横屏:action-bar 和 ad-banner 隐藏,display+keypad 并排
  .action-bar,
  .copy-submenu,
  .ad-banner {
    display: none;
  }
  // calc-display 和 calc-keypad 并排
  flex-direction: row;
  flex-wrap: wrap;
  // calc-display: 占左侧 40%
  .calc-display {
